...
% Find space??
% The compute cost of drawing a sample is more expensive than with a standard GAN\@.
% However, this expense is only incurred at test time; the typically more expensive training compute cost remains unchanged.
% Thus, the MH-GAN is best well-suited for applications where sample quality is more important then compute speed at test time.
...
We found a slight advantage for isotonic regression over other calibration methods.
...
If the AUC of $D$ is less than 0.9 we see acceptance rates over 20\%; but when the AUC of $D$ is 0.95, accept rates drop to 10\%.
