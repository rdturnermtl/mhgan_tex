...
\begin{align}
  \alpha = \min\left(1, \frac{\target(\vec x')\prop(\vec x_k \given \vec x')}{\target(\vec x_k)\prop(\vec x' \given \vec x_k)}\right)\,,
\end{align}
which for an independence sampler is:
\begin{align}
  \alpha = \min\left(1, \frac{\target(\vec x')\prop(\vec x_k)}{\target(\vec x_k)\prop(\vec x')}\right)\,.
\end{align}
...
The generator works by taking a sample from a Gaussian $\vec z \sample \norm(\vec 0,\mat I)$ and then transforming it with a generator network $G$: $\vec x = G(\vec z)$.
This implies a distribution on the data $\vec x \sample \PG$.  % TODO make lower
However, because $G$ is often a complex and non-invertible function, $\PG$ is generally intractable to compute.

The discriminator function $D$ is merely a standard binary classifier, that gives the probability a data point is real (as opposed to being sampled from $\PG$)\@.
...
If we assume that $D$ is an optimal discriminator for \emph{some} between the generator and some alternative distribution $\PD$, then:
\begin{align}
  D = \frac{\PD}{\PD + \PG} \implies \frac{\PD}{\PG} = \frac{1}{D^{-1}-1}\,, \label{eq:PD def}
\end{align}
and if $D$ is perfect then $\PD = \PR$.
We can use an MCMC independence sampler with a target distribution of $\PD$ and a proposal distribution of $\PG$.
This gives an acceptance probability of:
\begin{align}
  \alpha = \min\left(1, \frac{\PD(x')\PG(\vec x_k)}{\PD(\vec x_k)\PG(\vec x')}\right)
...
To test the calibration of the discriminator, we use a calibration test statistic defined in~\citet{Dawid1997} on a \emph{held out} data set:
% TODO define y
\begin{align}
  % TODO decide on notation N for size of calib set
  % np.sum(y_true - y_prob) / np.sqrt(np.sum(y_prob * (1.0 - y_prob)))
  Z := \frac{\sum_{i=1}^N y_i - D(\vec x_i)}{\sqrt{\sum_{i=1}^N D(\vec x_i) (1 - D(\vec x_i))}}\,. \label{eq:calib score}
\end{align}
...
% TODO didnt define init sample yt
...
This, of course, invalidates the guarantee that guarantee that $\vec x_K \sample \PR$.
...
\subsection{Toy Data}
We first demonstrate the MCMC GAN on the toy two Gaussians example presented in~\citet{Arjovsky2017}.
...
We consider the traditional DC-GAN~\citep{Radford2015} and as well as the popular WGAN~\citep{Arjovsky2017}\@.
...
However, applications where there is non-negligible overlap between $\PG$ and $\PR$ are perhaps the most interesting because proper calibration of the acceptance probabilities $\alpha$ are the most critical in these cases.
