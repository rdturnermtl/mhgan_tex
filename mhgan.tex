...
\todo{What is the reader supposed to make of the previous sentence? Clarify or remove.}
...
If we assume that $D$ is a Bayes' discriminator \jby{Is this an assumption? Or is it true by the definition of $\PD$?} between the generator $\PG$ and \emph{some} alternative distribution $\PD$ as per~\eqref{eq:PD inv}, we apply~\eqref{eq:alpha def} for a target of $\target=\PD$ and proposal $\prop=\PG$:
\begin{align}
...
\jby{As far as I can tell, Fig 2 does not show an illustration of sampling (it shows only marginals). Was this line perhaps written when we thought we'd make a Fig showing sampling?}
