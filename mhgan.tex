...
Each chain samples $\vec x_0 \sample \pinit$ and then does $K$ MH iterations to get $\vec x_K$ as the output of the chain, which is also the output of $G'$.
...
Furthermore, some GANs (like WGAN) required calibration because their discriminator only outputs a score and not a probability.
...
By initializing the chain at a sample of real data (the correct distribution), we can apply the detailed balance property and avoid burn-in.
...
However, MH-GAN is near perfect in its samples, while DRS still has many flawed samples.
...
\begin{center}
  \caption{{\small
    ...
    Here we compare 16 \emph{random samples} from the PGAN, calibrated (improved) DRS, and calibrated MH-GAN; where DRS and MH-GAN select from the same sequence of $G$ samples.
    There are 6 cases where the PGAN produces substantial warpings (red border) while the MH-GAN does not, and 2 cases where the MH-GAN does and the PGAN does not;
    for comparison with DRS, there are 7 where only DRS is warped, and 1 where only MH-GAN is warped.
    Even with 16 samples, the MH-GAN is better under a one-sided pairwise trinomial test~\citep{Coakley1996} at $p=0.01$ vs DRS and $p=0.07$ vs PGAN\@.
    }}
  \label{tbl:pgan}
