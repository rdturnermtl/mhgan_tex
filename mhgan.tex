...
The discriminator function $D \in \setx \rightarrow [0,1]$ is merely a standard binary classifier, that gives the probability a data point is real (as opposed to being sampled from $\PG$)\@.
...
They use a RealNVP network as the proposal as invertibility is important for computation of the acceptance probability $\accept$.
...
\begin{align}
  \accept(\vec x', \vec x_k) = \min\left(1, \frac{\PD(x')\PG(\vec x_k)}{\PD(\vec x_k)\PG(\vec x')}\right)
