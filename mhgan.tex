...
% TODO say proposal
If we assume that $D$ is an optimal discriminator between the generator $\PG$ and \emph{some} alternative distribution $\PD$ as per~\eqref{eq:define PD}, we can apply~\eqref{eq:alpha def} for a target of $\target=\PD$:
\begin{align}
...
Our method is based on the premise that $D$ is essentially ``ahead'' of the $G$.
The principled MCMC setup stochastically selects among samples from $G$ to correct biases in the $G$.
