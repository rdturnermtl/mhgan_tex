...
Such an approach also leads to a very low acceptance probability, which leads~\citet{Azadi2018} to use a hack that shifts the logit scores by an arbitrary $\gamma$ factor, violating the motivating rejection sampling setup.
Such a hack is not needed in the MCMC approach partially because we use a calibrator on the discriminator to get more accurate probabilities for computing acceptance probabilities.
...
\begin{figure}
    ...
    DRS fails to fully shift the distribution because 1)~it does not use calibration and 2)~its $\gamma$ shift violates the validity of rejection sampling.
    }}
\end{figure}
