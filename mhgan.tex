...
Although, the progressive GAN sometimes produces near photo realistic images, which made it famous, it also produces many flawed nightmare like images.
...
\begin{algorithm}[tb]
   ...
   \label{alg:mhgan}
\begin{algorithmic}
   \STATE {\bfseries Input:} base generator $G$, discriminator $D$, real samples $\vec x$
   \STATE Assign random real sample to $\vec x$
   \FOR{$k=1$ {\bfseries to} $K$}
   \STATE Draw $\vec x'$ from $G$
   \STATE Draw $U$ from uniform
   \IF{$U \leq \frac{D(\vec x_k)^{-1} - 1}{D(\vec x')^{-1} - 1}$}
   \STATE $\vec x \leftarrow \vec x'$
   \ENDIF
   \ENDFOR
   \STATE Return $\vec x$  % TODO try return
\end{algorithmic}
\end{algorithm}
