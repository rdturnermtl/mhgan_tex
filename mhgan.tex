...
In practice $D$ is often ``ahead'' of $G$
\citep{Shibuya2017}, which motivates wrapping an imperfect $G$ to obtain an improved $G'$ by using a more advanced $D$.
...
%, which is usually in \emph{parameter space}.
However, in MH-GANs, we are sampling from the \emph{data} distribution, so
we may use a trick: just initialize the chain at a sample of real data (the correct distribution) to avoid burn-in.
...
% TODO: Ryan, is the previous sentence correct? I'm not sure; I just tried to reword directly what I thought you were trying to say.
...
% TODO: Ryan: reword (B). I don't get it.
...
% TODO: clarify this with numbers. but no space in short version.
