...
This approach is conceptually simpler at first, but complexities arise as it is necessary to find an upper-bound on $D$ over all possible samples. Because this is not possible, one must instead rely on estimation by drawing many pilot samples.
...
%$\gamma$ factor, violating the motivating rejection sampling setup's validity.
