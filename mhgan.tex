...
The MH-GAN draws samples from the distribution implicitly defined by a GAN's discriminator-generator pair, as opposed to sampling in a standard GAN which draws sample from the distribution defined by the generator.
...
If GAN training reaches its global optimum then this discriminator distribution $\PD$ is equal to the true distribution on the data ($\PD = \PR$)~\citep{Goodfellow2014}.

\jby{TODO: Fix super confusing previous para}
...
Each MCMC chain results in a single sample $\vec x_K$,
%, which is the output of $G'$.
and independent chains are used for multiple samples from $G'$.
...
Additionally, even if $\vec x_k$ is not exactly distributed according to $\target$, the Kullback-Leibler (KL) divergence between the implied density it is drawn from and $\target$ will always decrease as $k$ increases~\citep{Murray2008,Cover2012}.\footnote{The curious reader may wonder why we do not simply stop the chain after the first accept. In general allowing chain length $K$ to be conditioned in any way on the state of the chain (including which samples were accepted or rejected) has the potential to introduce bias to the samples~\citep{Cowles1999}.}
...
%If we assume that $D$ is a Bayes' discriminator
%\jby{Is this an assumption? Or is it true by the definition of $\PD$?}
%between the generator $\PG$ and \emph{some} alternative distribution $\PD$ as per~\eqref{eq:PD inv},
We apply~\eqref{eq:alpha def} for a target of $\target=\PD$ and proposal $\prop=\PG$:
\begin{align}
