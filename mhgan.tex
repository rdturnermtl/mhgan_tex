...
\textbf{GANs} implicitly model the data $\vec x$ via a synthetic data generator \smash{$G \in \R^{d} \rightarrow \setx$}: \smash{$\vec x = G(\vec z)$}, \smash{$\vec z \sample \norm(\vec 0, \vec I_{d})$}.
...
We can also detect miscalibration of $D$ using the statistic of~\citet{Dawid1997} on held out samples $\vec x_{1:N}$ and real/fake labels $y_{1:N} \in \{0,1\}^N$.
...
%\footnote{WGAN does not learn a typical GAN discriminator that outputs a probability, so calibration is actually required in this case.}
Results are computed at epoch 60, as in Fig.~\ref{fig:incep_by_epoch} error bars and p-values are computed using a paired t-test across inception score batches, and all results are significantly better than the baseline GAN at \smash{$p < 0.05$}:
