...
The MH-GAN samples from the distribution implicitly defined by the discriminator in a GAN\@.
...
We demonstrate results on common datasets, including CIFAR-10 and CelebA, using DCGAN and WGAN\@.
\end{abstract}
...
Computing the likelihood becomes as easy as sampling new data from the trained model.
...
\end{align}
so we only need to evaluate $D(\vec x)$.
...
Similar concurrent work from \citet{Azadi2018} is discriminator rejection sampling (DRS) for GANs, which performs rejection sampling of outputs of $G$ by using the probabilities given by $D$.
...
Because this is not possible, one must instead rely on trying to crudely estimate this bound by drawing many pilot samples.
Secondly, even if one were to find a good bound, the acceptance rate would become very low due to the high-dimensionality of the sampling space. This leads~\citet{Azadi2018} to use an extra $\gamma$ heuristic to shift the logit $D$ scores, making the model sample from a distribution different from $\PR$ even when $D$ is perfect.
We use MCMC instead, which was invented precisely as a replacement for rejection sampling in higher dimensions.
We further improve the robustness of MCMC via use of a \emph{calibrator} on the discriminator to get more accurate probabilities for computing acceptance rates.
...
Each MCMC chain results in a single sample $\vec x_K$, which is the output of $G'$.
...
Additionally, even if $\vec x_k$ is not exactly distributed according to $\target$, its Kullback-Leibler divergence to $\target$ will always decrease as $k$ increases~\citep{Murray2008,Cover2012}.
...
If we assume that $D$ is a Bayes' discriminator between the generator $\PG$ and \emph{some} alternative distribution $\PD$ as per~\eqref{eq:PD inv}, we apply~\eqref{eq:alpha def} for a target of $\target=\PD$ and proposal $\prop=\PG$:
\begin{align}
...
We first show an illustrative synthetic mixture model example followed by real data for image generation.

\subsection{Mixture of 25 Gaussians}

We consider the $5 \times 5$ grid of two-dimensional Gaussians used in~\citet{Azadi2018}, which has become a popular toy example in the GAN literature ~\citet{Dumoulin2016}.
...
In Figure~\ref{fig:jsd}, we see that the MH-GAN improves the JSD over DRS by $5 \times$ on average, meaning it acheives a much more balanced spread across modes.
...
We can see the selectors (such as MH-GAN) results in a wider spread of probability mass across background colors.
In the case of CIFAR-10, it enhances modes with animal-like outlines and vehicles.
...
The MH-GAN has great potential for extension: %TODO: fill in. e.g. larger networks/datasets, language tasks (or some non-image).
