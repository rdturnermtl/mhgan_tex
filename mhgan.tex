...
% editor prefers "(MGAN), a GAN that" but that will cost us a line
...
\begin{figure}[bhtp]
    ...
    \rl{The left figure is not explained properly. What are the axes? what are the dots? What do shades of color mean?}
...
If GAN training reaches its global equilibrium then this discriminator distribution $\PD$ is equal to the true distribution on the data ($\PR = \PD$; here $\PR$ refers to the unknown true distribution the data.)~\citep{Goodfellow2014}.
...
Section~\ref{sec:Methods} also discusses a simplified deterministic approximation to the MGAN\@.
...
% TODO consider bullet points for contributions??
...
\rl{And? Paragrah seems unfinished here. Does our method introduce less bias?}
