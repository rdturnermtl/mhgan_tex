...
& \\
% \includegraphics[width=15cm]{uber_ai_logo_bootleg.pdf} \\
~ &
...
\mysection{The MH-GAN Algorithm}
...
\begin{itemize}
\item Use G as the independence sampler
\item Compute acceptance probability using the following observations:
  \begin{enumerate}
  \item \begin{align}
    \frac{\PD}{\PG} = \frac{1}{D^{-1} - 1} \implies
    \accept(\vec x', \vec x_k) = \min\left(1, \frac{D(\vec x_k)^{-1} - 1}{D(\vec x')^{-1} - 1}\right)
  \end{align}
  \item If $D$ is perfect $\PD = \PR$ (perfect target distribution). If $D$ is pretrained then $\PD \approxeq \PR$ 
  \end{enumerate}
\end{itemize}
